...
{
	// Text settings
	d.text[0].font = GxFont("default");
	d.text[0].SetColor(c.textColor);
	d.text[0].shadow = c.textShadow;
	d.text[0].SetFlag(GX_TF_ELLIPSIS);

	d.text[1] = d.text[0];
	d.text[1].shadow = GxColor(0, 0);
	...
	// Create floating dock frame
	{
		...
		for(int i=0; i<2; ++i)
		{
			...
			canvas.SetColor(0,0,0,0.1f + (float)i*0.1f);
...
GxStyle::Colors::Colors()
{
	GetBlackScheme(*this);
}

GxStyle::Colors::Colors(GxStyle::Scheme scheme)
{
	switch(scheme)
	{
		case BLACK:	GetBlackScheme(*this); break;
		case WHITE: GetWhiteScheme(*this); break;
	};
}
...
GxStyle* GxStyleImp::singleton = NULL;
...
GxStyle* GxStyle::Get()
{
	return GxStyleImp::singleton;
}

void GxStyle::Set(const GxStyle& style)
{
	*GxStyleImp::singleton = style;
}

void GxStyleImp::Create()
{
	singleton = new GxStyle(GxStyle::BLACK);
}

void GxStyleImp::Destroy()
{
	delete singleton;
	singleton = NULL;
}
